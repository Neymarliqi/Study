...
const install = (Vue, vm) => {
	// console.log("API中的VM",vm)
	// console.log("API中的Vue",Vue)
	let api ={}
	// 获取广告轮播图信息
	api.getAdvert = params => vm.$u.get('/advertisingspace/advertising?space=1,2,3');
   ...
   // 获取动态列表
	...
	api.getFeedComments = params => vm.$u.get(`/feeds/${params.id}/comments`+(params.after?`?after=${params.after}`:''))
	// /feeds/:feed/comments/:comment
	// 指定某一条评论
	api.getoneFeedComment = params => vm.$u.get('/feeds/' + params.id + '/comments' + comment.id)
	...
	// 获取指定 ID 资讯 评论详情
	api.getNewComments = params => vm.$u.get(`/news/${params.id}/comments`+(params.after?`?after=${params.after}`:''))
